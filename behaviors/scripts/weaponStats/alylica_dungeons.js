export const alylicaDungeons = [
	...
	},
    {
        ...
        beforeEffect: ({ player, target, crit, iframes, sweptEntities }) => {
            ...
            if (rng(10)) {
                ...
                player.dimension.playSound("random.anvil_land", player.location, {volume: 0.2, pitch: 1.5});
        ...
        script: ({ system, target, sweptEntities }) => {
            system.run(() => {
                sweptEntities.forEach(e => {
                    if (e.hasTag("prevent_effect"))
                        e.removeTag("prevent_effect")
                });
        ...
        beforeEffect: ({ player, target, specialCheck, iframes, sprintKnockback }) => {
            ...
            if (specialCheck) {
                if (iframes && target.__daggerSecondHit ) {
                    ...
                    player.dimension.playSound("weapon.daggers.hit", player.location, { volume: 0.6 });
	...
	},
    {
        ...
        beforeEffect: ({ system, player, target, specialCheck, crit, sprintKnockback }) => {
            if (!crit && !sprintKnockback && specialCheck) {
                ...
                player.dimension.spawnParticle("dungeons:swirling", {x: loc.x, y: loc.y+1, z: loc.z});
                system.run(() => {
	...
	},
    {
        ...
        beforeEffect: ({ target, iframes }) => {
            ...
            const loc = target.location
            target.dimension.spawnParticle("dungeons:element_freeze", {x: loc.x, y: loc.y+1, z: loc.z})
            target.dimension.playSound("mob.player.hurt_freeze", target.location, { volume: 0.33 });
        },
	},
    {
        ...
        beforeEffect: ({ target, crit, iframes }) => {
            ...
            return {
                ...
                critParticle: doCommittedBonus && !crit ? "sweepnslash:magic_critical_hit_emitter" : undefined
            }
        },
	...
	},
    {
        ...
        beforeEffect: ({player, specialCheck}) => {
            if (specialCheck) {
                player.dimension.playSound("weapon.obsidian_claymore.hit", player.location, { pitch: 1.2 });
                player.dimension.playSound("attack.sweep", player.location, { pitch: 1.7 });
            }
            return {
                sweepVolume: 0
            }

...
import { system, world, MolangVariableMap, Entity, Player, BiomeTypes } from "@minecraft/server";
...
export function inventoryAddLore(source) {
	...
	while (slot--) {
		...
		const itemLore = existingLore.filter(line =>
			!filter(line)
		)
		...
		if (existingLoreString.includes(atkSpeedStr) && existingLoreString.includes(damageStr)) continue;
...
Entity.prototype.applyAttackKnockback = function (location, max_height = 1) {
    ...
    const vy = a * (d - 1) * (lambertWm1(-Math.exp(-(y_max * d * ln + a * d - a) / (a * (d - 1)))) + 1) / d;
    const e = Math.exp( (delta.y * d * ln + a + d * (vy - a)) / (a * (d - 1)) );
    const W = lambertW0( -(a * (d - 1) - vy * d) * e / (a * (d - 1)) );
    const time = ( -a * (d - 1) * W + delta.y * d * ln + a * (1 - d) + vy * d) / ( a * (d - 1) * ln );
...
Entity.prototype.applyImpulseAsKnockback = function (vector3) {
  ...
  try {
	  ...
	  this.applyKnockback(
		{ x: vel.x + (directionX * horizontalStrength), z: vel.z + (directionZ * horizontalStrength) }, 
...
export function selectiveParticle(location, dynamicProperty, dimension, particleId, map, offset={x:0,y:0,z:0}) {
	system.run(() => {
		try {
		    ...
		    for (const p of world.getAllPlayers()) {
		        if (p.getDynamicProperty(dynamicProperty) == true && p.dimension.id == dimension) {
	            	map?p.spawnParticle(particleId, offsetLocation, map):p.spawnParticle(particleId, offsetLocation);
		        }
		    }
...
export function healthParticle(target, damage) {
	...
	selectiveParticle(loc, "damageIndicator", dimension, "sweepnslash:damage_indicator_emitter", map);
}
...
export function getCooldownTime(player, baseAttackSpeed = 4) {
    ...
    const adjustedSpeed = Math.max(baseAttackSpeed * hasteMultiplier * miningFatigueMultiplier, 0.000005);
...
Entity.prototype.getBiome = function() {
    ...
    for (const biome of biomeTypes) {
        ...
        const biomeLocation = dimension.findClosestBiome(
            location,
            biome,
            searchOptions
        );
...
Object.defineProperty(Entity.prototype, "isUnderground", {get: function() {
	const { dimension, location } = this;
    const { min, max } = dimension.heightRange;

    if (location.y < min || location.y > max) return false;

    const blocks = dimension.getBlockFromRay(location, { x: 0, y: 1, z: 0 }, {
...
Object.defineProperty(Entity.prototype, "isFasterThanWalk", {get: function() {
...
export class Check {
	...
	static durability(player, equippableComp, item, stats) {
        ...
        durabilityComp.damage = Math.min(durabilityComp.damage + durabilityModifier, durabilityComp.maxDurability);
	...
	// Duh.
	static criticalHit(currentTick, player, target, stats, { damage, noEffect, forced } = {}, { sound = "entity.player.attack.crit", particle = "minecraft:critical_hit_emitter", offset = {x:0,y:0,z:0}, map } = {}) {
		if (this.inanimate(target, {
		...
		const isValid = (player.isFalling && !player.isOnGround && !player.isInWater && !player.isFlying && !player.isClimbing && !player.isRiding
			...
			|| (forced == true)
		if (!noEffect && isValid) {
			selectiveParticle(target.center({x:0,y:1,z:0}), "criticalHit", dimension, particle, map, offset);
			if (!(target instanceof Player && target.getGameMode() === "creative")) selectiveSound(player.location, "critSound", dimension, sound);
		}
		return isValid
	}
	
	// Duh 2
	static sprintKnockback(currentTick, player, target, stats, { damage, noEffect, forced } = {}) {
		if (this.inanimate(target, {
				excludeTypes: [
		...
		if (damage <= 0) return;
		const status = player.getStatus();
		const isValid = (this.specialValid(currentTick, player, stats) && status.sprintKnockbackValid && forced == undefined) || forced == true
		if (isValid && !noEffect) {
			status.sprintKnockbackHitUsed = true;
			player.dimension.playSound( "entity.player.attack.knockback", player.location, { volume: 0.7 });
		}
		return isValid
	}
	
	// Duh 2 Epsiode 2
	static sweep(currentTick, player, target, stats, { fireAspect, damage, level = 1, forced, location, scale = 3 } = {}, { sound = "entity.player.attack.sweep", particle = "sweepnslash:sweep_particle", offset = {x:0,y:0,z:0}, pitch = 1, volume = 1, map } = {}) {
		const debugMode = world.getDynamicProperty("debug_mode");
		const status = player.getStatus();
		
		if (!(stats?.sweep && this.specialValid(currentTick, player, stats) && status.critSweepValid) && forced == undefined) return { swept: false, commonEntities: [] };
		if (((!player.isOnGround || player.isRiding || player.isFasterThanWalk) && forced == undefined) || forced === false) return { swept: false, commonEntities: [] };
	    ...
	    const commonEntities = playerCenter.filter(entity => 
			...
			(
		        ...
		        (player.getDynamicProperty("excludePetFromSweep") && !entity.isTamed({excludeTypes: [ "minecraft:trader_llama" ]}))
		    )
	    );
		...
		selectiveParticle(location || particleLocation, "sweep", dimension, particle, map, offset);
		player.dimension.playSound(sound,pLoc,{pitch, volume});
	
	    commonEntities.forEach(e => {
			let dmgType = this.shieldBlock(currentTick, player, e, stats, { disable: true })?"entityExplosion":"entityAttack";
	...
	static shieldBlock(currentTick, player, target, stats, { disable = false } = {}) {
		...
		if (world.getDynamicProperty("shieldBreakSpecial") && player instanceof Player) specialValid = Check.specialValid(currentTick, player, stats);
    ...
    // Self explanatory
    static calculateDamage(player, target, item, stats, currentTick, timeSinceLastAttack, baseDamage, attackSpeed, { damageTest = false, critAttack, critMul, cancel, attackSpeedTicks } = {}) {
	    ...
	    const crit = this.criticalHit(currentTick, player, target, stats, { noEffect: true, forced: critAttack }) && target.typeId != "minecraft:ender_dragon" ? (critMul??1.5) : 1;
		...
		const isInRain = !target.isUnderground && target.dimension.getWeather() !== "Clear" && !biomeArray.includes(target.getBiome()?.id)
	    ...
	    if (enchantBonus > 0 && !this.inanimate(target, {
				excludeTypes: [
					"minecraft:armor_stand",
					"minecraft:boat",
					"minecraft:chest_boat",
	    ...
	    const damage = (((baseDamage + (strengthModifier - weaknessModifier))) * crit * multiplier) + enchantDamage;
	...
	static finalDamageCalculation(currentTick, player, target, item, stats, { damage, critAttack, critMul, cancel }) {
		const status = player.getStatus();
		...
		const calculateDamage = this.calculateDamage(player, target, item, stats, currentTick, timeSinceLastAttack, baseDamage, attackSpeed, { critAttack, critMul, cancel });
	...
	static view(player) {
		const targetEntity = player.getEntitiesFromViewDirection({
			maxDistance:3,
			excludeTypes: [ "item", "xp_orb", "arrow", "ender_pearl", "snowball", "egg", "painting", "tnt", "fishing_hook", "falling_block", "ender_crystal", ]
			})[0]?.entity;
			
		if (targetEntity && this.inanimate(targetEntity, {excludeTypes: [ "minecraft:armor_stand" ]} )) {
			return null;
		}
	...
	static damageTest(player) {
        ...
        for (let t = 0; t <= attackSpeedTicks; t++) {
            const damage = this.calculateDamage(player, player, item, stats, currentTick, t, baseDamage, attackSpeed, { damageTest: true, attackSpeedTicks }).damage;
            damageLog.push(`§f[ §a${t} (${Math.round(t*100/attackSpeedTicks)}%) §f| §c${damage.toFixed(2)} §f| §e${(damage/(t/20)).toFixed(2)} (${(damage/Math.max(0.5,(t/20))).toFixed(2)}) §f]`);
        }

        if (debugMode) debug(`${item?.typeId || "hand"} ${stats || item == undefined ? "" : "§c(Weapon stats not found)§f"}\n[ §aTicks §f| §cDamage §f| §eDPS (with iframes) §f]\n§e${damageLog.join("\n")}`);
    }
}
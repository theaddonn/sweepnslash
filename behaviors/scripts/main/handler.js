...
import { world, system, Player, CustomCommandStatus, CustomCommandSource } from "@minecraft/server";
...
let SimulatedPlayer;
let gametest = true;
import("@minecraft/server-gametest").then(module => {
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
	...
	if (initialSpawn) {
		...
		if (player.getDynamicProperty("tipMessage") === undefined || player.getDynamicProperty("tipMessage"))
			player.sendMessage({
				rawtext: [
					{
...
function configForm(player) {
	...
	if (op == true) {
	    ...
	    if (!world.isHardcore) form.toggle({ translate: "sweepnslash.toggleaddon" }, { defaultValue: dp(world, { id: "addon_toggle" })});
		form.toggle({ translate: "sweepnslash.shieldbreakspecial" }, { defaultValue: dp(world, { id: "shieldBreakSpecial" }), tooltip: { translate: "sweepnslash.shieldbreakspecial.tooltip" }});
	    form.toggle({ translate: "sweepnslash.toggledebugmode" }, { defaultValue: dp(world, { id: "debug_mode" }), tooltip: { translate: "sweepnslash.toggledebugmode.tooltip" }});
		form.divider();
	}
	
	form.label({ translate: "sweepnslash.generaltoggleheader" });
	form.toggle({ translate: "sweepnslash.excludepetfromsweep" }, { defaultValue: dp(player, { id: "excludePetFromSweep" }) ?? false, tooltip: { translate: "sweepnslash.excludepetfromsweep.tooltip" }});
	form.toggle({ translate: "sweepnslash.tipmessagetoggle" }, { defaultValue: dp(player, { id: "tipMessage" }) ?? false });
	form.divider();
	...
	form.dropdown(
	    ...
	    { defaultValueIndex: dp(player, { id: "cooldownStyle" }), tooltip: { translate: "sweepnslash.indicatorstyle.tooltip" }}
	);
	form.toggle({ translate: "sweepnslash.bowhitsound" }, { defaultValue: dp(player, { id: "bowHitSound" }) ?? false });
	form.toggle({ translate: "sweepnslash.sweepparticles" }, { defaultValue: dp(player, { id: "sweep" }) ?? false });
	form.toggle({ translate: "sweepnslash.enchantedhitparticles" }, {defaultValue: dp(player, { id: "enchantedHit" }) ?? false });
	form.toggle({ translate: "sweepnslash.damageindicatorparticles" }, { defaultValue: dp(player, { id: "damageIndicator" }) ?? false });
	form.toggle({ translate: "sweepnslash.critparticles" }, { defaultValue: dp(player, { id: "criticalHit" }) ?? false });
	form.toggle({ translate: "sweepnslash.critsounds" }, { defaultValue: dp(player, { id: "critSound" }) ?? false });
	form.divider();
	...
	form.show(player).then((response) => {
		...
		const properties = [
			{ object: world, dynamicProperty: "addon_toggle", condition: op && !world.isHardcore },
...
system.runInterval(() => {
	...
	for (const player of world.getAllPlayers()) {
		...
		if (
			(player.selectedSlotIndex !== status.lastSelectedSlot && status.lastSelectedItem !== item?.typeId) ||
			(status.lastSelectedItem !== item?.typeId && !(status.lastSelectedItem === undefined && item?.typeId === undefined))
		) {
		...
		if (player.isFalling && !player.isGliding && !player.isOnGround && !player.isInWater && !player.isFlying && !player.isClimbing
		...
		if (!addonToggle || barStyle === 3) {
		    ...
		    if (curCD > 0 || (viewCheck && stats && barStyle === 0)) {
		        barStyle !== 2 ? 
					player.onScreenDisplay.setTitle(`_sweepnslash:${barArray}:${bonkReady ? 't' : 'f'}:${uiPixelValue}`, {
						fadeInDuration: 0,
						fadeOutDuration: 0,
						stayDuration: 0
					})
...
system.afterEvents.scriptEventReceive.subscribe(({id, sourceEntity: player}) => {
    if (world.getDynamicProperty("addon_toggle") == false
...
world.afterEvents.projectileHitEntity.subscribe((event) => {
	...
	if (configCheck && (target instanceof Player) && player !== target && projectile.typeId === "minecraft:arrow") {
...
world.afterEvents.entityHitEntity.subscribe(({damagingEntity: player, hitEntity: target}) => {
	...
	player.__leftClick = true;
    if (target?.isValid && player?.getComponent("health")?.currentValue > 0) CombatManager.attack({ player, target, currentTick })
});
...
world.afterEvents.entityHurt.subscribe(({ damageSource, hurtEntity, damage }) => {
	...
	if (player instanceof Player) {
		if (damageSource.cause === "entityAttack") {
			...
			//if (!shieldBlock)
			hurtEntity.__lastAttack = { rawDamage: player.__rawDamage, damage: damage, time: currentTick };
			healthParticle(hurtEntity, damage);
		} else if (damageSource.cause === "maceSmash") {
			healthParticle(hurtEntity, damage);
		} else {
			hurtEntity.__lastAttack = { rawDamage: damage, damage: damage, time: currentTick };
		}
	} else {
		hurtEntity.__lastAttack = { rawDamage: damage, damage: damage, time: currentTick };
	}

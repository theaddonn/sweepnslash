...
export class CombatManager {
	static attack(eventData) {
		...
		let hit = false;
		let dmg = baseDamage;
		let crit = Check.criticalHit(currentTick, player, target, stats, { noEffect: true });
		let sprintKnockback = Check.sprintKnockback(currentTick, player, target, stats, { noEffect: true });
		let sweep = Check.sweep(currentTick, player, target, stats);
		let specialCheck = Check.specialValid(currentTick, player, stats);

		// Iframes check
		const lastAttack = target.__lastAttack || { rawDamage: 0, damage: 0, time: 0 };
		const timeElapsed = currentTick - lastAttack.time;
		const timeValid = timeElapsed >= 10;
		
		const beforeEffect = stats?.beforeEffect?.({ 
			mc, system, world, player, target, item, dmg, specialCheck, sweptEntities: sweep?.commonEntities, crit, sprintKnockback, cooldown, iframes: !timeValid
		}) || {};
		...
		crit = Check.criticalHit(currentTick, player, target, stats, { 
			damage: dmg.final, forced: beforeEffect?.critAttack 
		}, { 
		...
		sprintKnockback = Check.sprintKnockback(currentTick, player, target, stats, { 
			damage: dmg.final, forced: beforeEffect?.sprintKnockback 
		});
		
		const fireAspect = Check.enchantLevel(item, "fire_aspect");
		const inanimate = Check.inanimate(target, { excludeTypes: ["minecraft:armor_stand"] });
		const knockback = (Check.enchantLevel(item, "knockback") ?? 0) + (sprintKnockback ? 1 : 0);
		...
		*/
		...
		player.startItemCooldown("minecraft:shield", player.getItemCooldown("minecraft:shield") || 1);
		status.lastShieldTime = currentTick;
		const shieldBlock = Check.shieldBlock(currentTick, player, target, stats, {disable: true});
		...
		const applyKnockback = (knockbackLevel, pLoc, tLoc, rot) => {
			...
			const dirX = knockbackValid
				? -Math.sin(rot.y*(Math.PI/180))
				: tLoc.x - pLoc.x;
			const dirZ = knockbackValid
				? Math.cos(rot.y*(Math.PI/180))
				: tLoc.z - pLoc.z;
		...
		const iframes = (timeValid || (dmg.raw > lastAttack.rawDamage && dmg.final > lastAttack.damage))
						&& !((target instanceof Player) && target.getGameMode() == "creative")
		if (iframes) {
			// Apply sweeping
			sweep = Check.sweep(currentTick, player, target, stats, {
				fireAspect, damage: dmg.final, level: beforeEffect?.sweepLevel, forced: beforeEffect?.sweep, location: beforeEffect?.sweepLocation, scale: beforeEffect?.sweepRadius
			}, {
			...
			const damageValid = status.mace === true && player.isFalling ? false : target.applyDamage(dmg.final, { cause: dmgType, damagingEntity: player });
			hit = damageValid;
			...
			if (status.mace === false && !inanimate && dmg.final > 0 && !beforeEffect?.cancelDurability) {
			...
			if (damageValid && !shieldBlock) {
				...
				} catch (e) { if (debugMode) debug("Error during knockback: " + e + ", knockback skipped"); }
			}

			if (dmg.final > 0) {
				if (dmg.enchantedHit) selectiveParticle(target.center({ x: 0, y: 1, z: 0 }), "enchantedHit", targetDimensionID, "sweepnslash:magic_critical_hit_emitter")
				if (!(sweep?.swept || crit)) {
					player.dimension.playSound(specialCheck ? "game.player.attack.strong.se" : "game.player.attack.weak.se", loc, { volume: 0.7 });
				}
			}
		} else {
			if (dmg.final > 0) player.dimension.playSound("game.player.attack.nodamage.se", loc, { volume: 0.7 });
		}

		if (debugMode) {
			debug(`${player.name}'s §pDamage result:
§f- Attacked with:§e ${item?.typeId ?? "hand"} ${stats||item == undefined ? "" : "§c(Weapon stats not found)"}
§f- Attempted damage: §e${dmg.final.toFixed(2)} (${(cooldown*100).toFixed(0)}%) ${specialCheck ? "§a+" : ""} ${iframes?"":"(iframes immunity)"}
§f- Ticks since last attack: §e${currentTick - status.lastAttackTime}`);
		...
		if (stats?.script) {
			system.run(() => {
				stats.script({ mc, system, world, player, target, item, sweptEntities: sweep?.commonEntities, dmg, hit, shieldBlock, specialCheck, crit, sprintKnockback, inanimate, cooldown });
			});
		}
	}
};